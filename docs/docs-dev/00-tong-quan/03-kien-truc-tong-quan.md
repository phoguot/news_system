...
|---|---|---|---|
| `Frontend` | `Frontend\` | Trang đọc cho khách: Home, Post, Category, Tag, Search, Service, Team, Contact, Sitemap, About, Pricing; layout đọc menu động từ `menu_items` qua helper `frontendMenu` | `/`, `/tin-tuc`, `/danh-muc/{slug}`, `/tag/{slug}`, `/tim-kiem`, `/dich-vu`, `/dich-vu/{slug}`, `/doi-ngu`, `/lien-he`, `/gioi-thieu`, `/bang-gia`, `/sitemap.xml`, `POST /api/contact` |
| `Admin` | `Admin\` | CMS cho quản trị viên: Auth, Dashboard, Post, Category, Tag, Service, Banner, HomeSection, Menu, Team, Contact, Media, Setting, Account, Api | `/admin/*` (trang) + `/api/admin/:resource[/:id][/:sub]` (JSON) |
...
| Menu frontend (17/09) | `Frontend\Service\MenuService::publicItems()` cache key `menu-v1` TTL 60s, đọc `menu_items` active theo `sortOrder`; layout gọi helper `frontendMenu`. Ghi phía Admin (`Admin\Service\MenuService`) forget `menu-v1`; helper có fallback menu mặc định khi DB chưa migrate hoặc storage lỗi |
